...
/// Key/value tensors after cache lookup or update.
///
/// `QuantizedKVCache.update` fatal-errors; owning layers must use
/// `updateQuantized`, and shared layers must read `getQuantizedState`.
private enum Gemma3nKVState {
...
class Gemma3nAttention: Module {
    ...
    func callAsFunction(
        ...
        let kvState: Gemma3nKVState
        if isKvSharedLayer, let cache, let existing = existingKVState(from: cache) {
            kvState = existing
        } else {
            kvState = storeKV(
                keys: projectedKeys(x, batch: B, length: L, offset: offset),
                values: projectedValues(x, batch: B, length: L),
                cache: cache
            )
        }
        ...
        let adjustedMask = gemma3nAdjustedAttentionMask(
            mask,
            keySequenceLength: kvState.sequenceLength
        )
    ...
    private func projectedKeys(
        _ x: MLXArray, batch: Int, length: Int, offset: RoPEOffset?
    ) -> MLXArray {
        var keys = kProj(x).reshaped(batch, length, -1, headDim)
        keys = kNorm(keys)
        keys = keys.transposed(0, 2, 1, 3)
        return applyRotaryPosition(rope, to: keys, offset: offset)
    }

    private func projectedValues(_ x: MLXArray, batch: Int, length: Int) -> MLXArray {
        var values = vProj(x).reshaped(batch, length, -1, headDim)
        values = vNorm(values)
        return values.transposed(0, 2, 1, 3)
    }

    private func existingKVState(from cache: KVCache) -> Gemma3nKVState? {
        if let quantizedCache = cache as? QuantizedKVCacheProtocol {
            guard let (quantizedKeys, quantizedValues) = quantizedCache.getQuantizedState() else {
                return nil
            }
            return .quantized(
                keys: quantizedKeys,
        ...
        return .regular(keys: state[0], values: state[1])
    }

    private func storeKV(keys: MLXArray, values: MLXArray, cache: KVCache?) -> Gemma3nKVState {
        if let quantizedCache = cache as? QuantizedKVCacheProtocol {
        ...
        if let cache {
            ...
            return .regular(keys: updatedKeys, values: updatedValues)
        }
